...
public class LazersScenario implements bonzai.Scenario {
	...
	public LazersScenario(File file, int teamID) throws IOException {
...
//			LazersRenderer.setMapSize(Integer.parseInt(size[0]), Integer.parseInt(size[1]));
	...
	public Game instantiate(List<? extends bonzai.Jar> jars) {
		...
		LazersMap map = new LazersMap(this.map);
		...
		Game game = new Game(teamID, LazersScenario.NUM_TURNS, map, colors);
