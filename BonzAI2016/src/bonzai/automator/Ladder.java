...
public class Ladder {	
	public static void main(String[] args) throws FileNotFoundException {
		...
		Stack<bonzai.Jar> aiStack = new Stack<bonzai.Jar>();
		Stack<bonzai.Jar> discarded = new Stack<bonzai.Jar>();
		...
		for (int round = 0; round <= 3; round++) {
			...
			for (int level = 0; level < 3; level++) { //high,med,low
				...
				for (int i = 0; i < ais.get(round).size(); i+=3) {
					...
					try {
						String exec = "java -jar snowbound.jar -run " + scenarios.get(map) + jarString;
	...
	public static void saveState(Stack<bonzai.Jar> remaining, Stack<bonzai.Jar> discarded) {
		...
		//Jars go from teh remaining stack t the discarded stack
