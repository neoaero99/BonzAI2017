...
/**
 * A minimum ordered adaptable priority queue (PQ) of key-value pair entries,
 * sorted based on keys of type, K. The backend of the PQ is a minimum heap
 * array. Also, the entries of the PQ are position aware.
 * 
...
public class MinPQ<K extends Comparable<K>, V> {
	...
	/**
	 * Returns the value of entry, whose key is the least of all the PQ
	 * entries.
	 * 
	 * @return	The value of the entry with the least key value
	...
	/**
	 * Returns the value of the entry, whose key is the least of all entries in
	 * the PQ and removes the entry from the PQ.
	 * 
	 * @return	The value of the entry with the least key value
	 * @throws 	PQOpException- if the PQ is empty
	 */
	 ...
	 * children, picking the child with the lesser key to swap positions with
	 * the entry, if that child's key is less than that of the entry. Continue
	 * this process until both of the entry's children's keys are greater than
	 * that of the entry, or the end of the heap is reached.
	 * 
